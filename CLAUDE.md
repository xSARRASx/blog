...
- **YOUTUBE / blocage du 17/09/2026, nouvelle piste testée et ÉCARTÉE** : yt-dlp 2026.08.19 affiche désormais un WARNING inédit `Remote component challenge solver script (node) was skipped... enable with --remote-components ejs:github`. Testé : `--remote-components ejs:github` ne change RIEN, le blocage reste `Sign in to confirm you're not a bot`. Idem avec le script PO token bgutil passé explicitement (`--extractor-args "youtubepot-bgutilscript:script_path=/root/bgutil-ytdlp-pot-provider/server/build/generate_once.js"`) et avec `player_client=android,mweb,tv`. Un `HTTP Error 429` accompagne systématiquement la première requête webpage. Bilan : l'environnement local est complet (Node détecté, PO token généré, EJS activé), il ne manque QUE l'identification. **Seul correctif restant : `YT_COOKIES`.** Ne pas repasser de temps sur les autres pistes.
- **YOUTUBE / 21/09/2026 : le blocage a CHANGÉ DE NATURE, et c'est une bonne nouvelle.** Sur la vidéo ZdEMuAtcJcE, la commande de référence ne renvoie PLUS `Sign in to confirm you're not a bot` : yt-dlp accède à la vidéo, choisit le client `visionos` et ANNONCE les sous-titres (`Downloading subtitles: fr-orig, fr`). L'échec survient une étape plus loin, au téléchargement du fichier lui-même, en `HTTP Error 429` sur le serveur timedtext. Autrement dit ce n'est plus l'anti-bot mais un simple rate-limit, donc **une relance quelques heures plus tard a de vraies chances d'aboutir**. Constaté aussi : `--sleep-subtitles`, `--retries` et `--retry-sleep` ne suffisent pas (le 429 est immédiat), et insister fait REGRESSER vers le `Sign in to confirm` (3e tentative). Conduite à tenir inchangée : 2-3 tentatives, aucun article sans transcription, aucune entrée dans traite.json, relance programmée.
...
| article-frais-airbnb.html | LCD | frais airbnb (post 10382) | Fourche à deux chemins de réservation avec taux comparés (PREMIÈRE utilisation), Donut de décomposition des 15,5 % en acquisition et technologie (PREMIÈRE utilisation), Mockup de curseur d'interface avec avertissement (PREMIÈRE utilisation), Parcours 3 profils de voyageur reliés par flèches avec taux dégressif, Baromètre de dépendance en dégradé avec curseur et 3 zones, CTAs duo |
| article-faux-riches.html | LCD (Dev perso) | faux riches (post 10384) | Grille de feed social 6 publications avec étiquettes Loué (PREMIÈRE utilisation), Chiffre annoncé encadré de 4 cartes-questions numérotées, Graphique en courbes SVG annoncé vs réel (PREMIÈRE utilisation), Trois voyants ronds temps/téléphone/agenda (PREMIÈRE utilisation), Liste barrée ce que j'ai arrêté avec bénéfice en regard (PREMIÈRE utilisation), CTAs duo |
| article-residence-principale.html | LCD | résidence principale (post 10386) | Compteur à aiguille en demi-cadran avec zone rouge (PREMIÈRE utilisation), Fiche signalétique portrait-robot 6 indices cochés + bandeau verdict (PREMIÈRE utilisation), Barres horizontales décroissantes avec total encadré orange, Maison coupée en deux par un trait pointillé exonéré/taxé (PREMIÈRE utilisation), Frise de délais bicolore avec légendes sous segments + 2 cartes d'échéance, CTAs duo |
| article-controle-fiscal.html | LCD | contrôle fiscal (post 10388) | Chaîne DAC7 : 4 plateformes vers un email unique dupliqué chez deux destinataires (PREMIÈRE utilisation), Règle segmentée d'un virement avec accolade orange sur la part à déclarer (PREMIÈRE utilisation), Deux portails de seuils cotisations vs statut LMP (PREMIÈRE utilisation), Double frise des fenêtres de contrôle 3 ans / 6 ans (PREMIÈRE utilisation), Escalier ascendant des majorations 10/40/80 %, CTAs duo |
| article-marche-immobilier.html | LCD | marché immobilier (post 10390) | Tableau de bord cockpit 6 indicateurs avec micro-barres et badges de tendance (PREMIÈRE utilisation), Graphique SVG du tunnel de Friggit avec bande d'équilibre et deux courbes divergentes (PREMIÈRE utilisation), Chaîne à 3 maillons dont un grippé en orange (PREMIÈRE utilisation), Double réservoir vidé en jauges verticales taux et durée (PREMIÈRE utilisation), Croisement inversé des courbes État et crédit avec zone d'anomalie hachurée (PREMIÈRE utilisation), Trois scénarios en curseurs de probabilité avec conséquence chiffrée (PREMIÈRE utilisation), Liste de 4 seuils déclencheurs avec valeur pivot et action (PREMIÈRE utilisation), CTAs duo |
| article-residence-fiscale.html | LCD | résidence fiscale (post 10392) | Compteur 183 barré d'un trait orange avec duel 144 jours / verdict (PREMIÈRE utilisation), Quatre interrupteurs dont un seul basculé (PREMIÈRE utilisation), Barre de barème 20 %/30 % avec pivot 29 000 euros et deux cartes de prélèvements (PREMIÈRE utilisation), Fil tendu entre l'expatrié et le fisc avec 3 nœuds d'attache (PREMIÈRE utilisation), Double étiquette de prix vendu avant / vendu après (PREMIÈRE utilisation), Hémicycle de 577 sièges avec score 131/132 et écart d'une voix (PREMIÈRE utilisation), Cinq tampons de passeport inclinés en checklist (PREMIÈRE utilisation), CTAs duo |
| article-gestion-airbnb.html | LCD | gestion airbnb (post 10394) | Entonnoir à 3 couches avec pourcentage absorbé par couche (PREMIÈRE utilisation), Registre d'interruptions en tableau avec colonne « ce qui aurait évité l'appel » et badges de nature (PREMIÈRE utilisation), Deux bulles de message face à face, pavé grisé avec code surligné contre message d'accès court (PREMIÈRE utilisation), Rail vertical du parcours voyageur avec badge de canal par étape (PREMIÈRE utilisation), Double colonne réponse automatique / passage à l'humain sur fond navy (PREMIÈRE utilisation), Trois cartes de délégation à pastille et badge de délai (PREMIÈRE utilisation), Plan 30 jours en 4 colonnes semaine avec livrable nommé (PREMIÈRE utilisation), CTAs duo |
